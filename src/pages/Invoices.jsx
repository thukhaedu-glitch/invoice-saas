  ...
  const handleSavePayment = async () => {
    ...
    try {
      ...
      await logAction(companyId, {
        ...
        description: `Payment recorded: ${item.invoiceNumber} — ${item.clientName} — ${money(amount)}`,
          ...
          <div style={{ background: 'white', borderRadius: 16, width: '100%', maxWidth: 460, boxShadow: '0 20px 60px rgba(0,0,0,0.2)', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ padding: '20px 24px', borderBottom: '0.5px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>
                  {paymentModal.invoiceNumber} — Total: {money(paymentModal.totalAmount || 0)}
                  {paymentModal.paidAmount>0 && <span style={{ color: '#16a34a' }}> | Paid: {money(paymentModal.paidAmount)}</span>}
                  {paymentModal.remainingAmount>0 && <span style={{ color: '#d97706' }}> | Due: {money(paymentModal.remainingAmount)}</span>}
                </div>
              </div>
            ...
            <div style={{ padding: 24 }}>
              <div style={{ marginBottom: 12 }}>
                <label style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-2)', display: 'block', marginBottom: 4 }}>Amount ({currency.symbol}) *</label>
                ...
                <div style={{ marginBottom: 16, padding: 12, background: '#f8fafc', borderRadius: 8 }}>
                  ...
                  {paymentModal.payments.map((p, i) => (
                    <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, padding: '4px 0', borderBottom: '0.5px solid #e2e8f0' }}>
                      ...
                      <span style={{ fontWeight: 500, color: '#16a34a' }}>{money(p.amount)}</span>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div className="stats-grid-4" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 16 }}>
        {statsCards.map(({ label, value, amount, color, bg }) => (
          <div key={label} className="card" style={{ padding: 16 }}>
            ...
            <div style={{ fontSize: 12, color, fontWeight: 500 }}>{money(amount)}</div>
          </div>
        ))}
      </div>
