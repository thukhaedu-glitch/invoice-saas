                    ...
                    <tr key={item.id} style={{ background: isSelected ? 'rgba(79,110,247,0.06)' : item.status === 'pending_approval' ? 'rgba(79,110,247,0.03)' : item.status === 'admin_approved' ? 'rgba(22,163,74,0.03)' : '' }}>
                      ...
                      <td style={{ textAlign: 'center' }}>
                        <div style={{ display: 'flex', gap: 4, justifycontent: 'center', alignItems: 'center' }}>
                          ...
                          {['pending', 'partial', 'overdue'].includes(item.status) && (
                            <button type="button" onClick={() => handleSendReminder(item)} disabled={sendingReminder === item.id} title="Send Email Reminder" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#d97706', padding: 4, borderRadius: 6, opacity: sendingReminder === item.id ? 0.5 : 1 }}>
                              <Mail size={14} />
                            </button>
                          )}

                          {canDelete && (
                            <button type="button" onClick={() => handleDeleteWithAuth(item.id)} title="Delete" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--danger)', padding: 4, borderRadius: 6 }}><Trash2 size={14} /></button>
                          )}
                        </div>
                      </td>
                    </tr>
