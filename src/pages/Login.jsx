...
const saveMemberProfile=async(cid,uid,email)=>{
try{
await setDoc(doc(db,'companies',cid,'memberProfiles',uid),{
uid,email,lastLogin:new Date().toISOString(),
},{merge:true})
}catch(e){console.error('memberProfile:',e)}
}
...
await saveMemberProfile(cid,cred.user.uid,cred.user.email)
}
navigate('/')
...
<div style={{width:52,height:52,background:'var(--primary)',borderRadius:14,display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px',boxShadow:'0 4px 16px rgba(79,110,247,0.3)'}}>
<LogIn size={24} color="white"/>
</div>
<div style={{fontSize:20,fontWeight:700,color:'var(--text-1)'}}>{resetMode?'Reset Password':'Welcome Back'}</div>
<div style={{fontSize:13,color:'var(--text-3)',marginTop:4}}>{resetMode?'Enter your email to reset':'Sign in to AnkoraX'}</div>
</div>
{error&&(
...
<button type="button" onClick={()=>setResetMode(true)} style={{background:'none',border:'none',color:'var(--primary)',fontSize:12,cursor:'pointer'}}>Forgot password?</button>
</div>
)}
...
{resetMode&&(
<button type="button" onClick={()=>{setResetMode(false);setResetSent(false);setError('')}} className="btn btn-ghost" style={{width:'100%',padding:'11px',fontSize:14,justifyContent:'center'}}>Back to Login</button>
)}
</form>
)}
{!resetMode&&!resetSent&&(
